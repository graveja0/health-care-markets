...
specialty_groupings <-
  list(
    cardiology = c("CAR", "PDC"),
    primcare = c("FMP", "GNP", "INT"),
    orthsurg = c("ORC", "ORS", "OSN", "OFA"), 
    behavioral = c("PSY","PSC"),
    pediatrics = c("PED","IMP"), 
    oncology = c("ONC","GYO","PHO","RDO","ONL")
  )
...
p1 <- sf_county %>% 
  left_join(df_md_hhi_county %>% filter(year==2017),"fips_code") %>% 
...
p2 <- sf_cz %>% 
  left_join(df_md_hhi_cz %>% filter(year==2017),"cz_id") %>% 
...
p3 <- sf_hrr %>% 
  left_join(df_md_hhi_hrr %>% filter(year==2017),"hrrnum") %>% 
...
p4 <- sf_ra %>% 
  left_join(df_md_hhi_ra %>% filter(year==2017),"rating_area") %>% 
  filter(state %in% states_to_map) %>% 
  ggplot() + 
  geom_sf(aes(fill = hhi_primcare)) +
  scale_fill_gradient2(low = scales::muted("blue"),mid = "white",high = scales::muted("red"),midpoint = 2500,limits = c(0,10000)) + 
  ...
  coord_sf(datum=NA) + 
  remove_all_axes +
  ggtitle("Primcare Care\nRating Area") + 
  ggthemes::theme_tufte(base_family = "Gill Sans")

p5 <- sf_pcsa %>% 
  left_join(df_md_hhi_pcsa %>% filter(year==2017),"pcsa") %>% 
