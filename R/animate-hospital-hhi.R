...
hhi_chg <- 
  ...
  mutate(chg_2013 = hhi_2013 - hhi_2013,
         chg_2014 = hhi_2014 - hhi_2013,
         chg_2015 = hhi_2015 - hhi_2013,
         chg_2016 = hhi_2016 - hhi_2013,
         chg_2017 = hhi_2017 - hhi_2013) %>% 
  select(cz_id,contains("chg_")) %>% 
  gather(key,value,-cz_id) %>% 
  mutate(year = as.numeric(paste0(gsub("chg_","",key))))


states_to_map = c("TN","KY","NC","WV","VA")
p2 <- 
sf_cz %>% 
