---
title: "Obtaining Population-Weighted County Centroids"
output:
  html_document:
    df_print: paged
editor_options: 
...
counties_to_show <- cent_county %>% 
  #filter(row_number() %in% unlist(st_intersects(sf_county, sf_county))) %>% 
  pull(fips_code)
...
sf_county_wtd <- 
  int %>% 
  mutate(fips_code = sf_county$fips_code[int$row.id]) %>% 
...
sf_county %>% 
  ...
  geom_sf(alpha = 0) + 
  remove_all_axes + 
  geom_sf(data = sf_county %>% filter(row_number()==int$row.id[which_zip]),colour = "darkred", alpha =0.1,lwd=0.5) +
  geom_sf(data = sf_tract %>% filter(row_number() %in% get_first), colour = "black",alpha = 0,lwd=.75) +
  geom_point(data = sf_county_wtd , aes(x=x,y=y) ,colour="black") + 
  geom_point(data = cent_county,aes(x=x,y=y),colour="red",pch=3) + 
