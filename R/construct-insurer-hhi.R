...
df_drg<- data.table::fread(file.path(fs::path_home(),"box/Research-Provider_Networks/aim1-breadth/input/drg/drg-mms-2016-2017.csv")) %>% 
  ...
  filter(month==7) %>% 
  filter(company_name=="") %>% 
  select(fips_code,mco_name,parent_id,total_book,commercial,comm_si, comm_fi,medicare,medicaid,phxind,phxshop) %>% 
  ...
  gather(market,enrollment,-fips_code,-mco_name,-parent_id) %>%
  unique() %>% 
  group_by(fips_code,market) %>% 
  mutate(enrollment = as.numeric(paste0(enrollment))) %>% 
  ...
  select(market,hhi) %>% 
  unnest()  %>% 
  gather(var,value,-fips_code,-market) %>% 
...
drg_hhi_aggregated <-
  df_drg %>% 
  gather(variable,value,-fips_code) %>% 
...
p1 <- sf_county %>% 
  left_join(drg_hhi_county ,"fips_code") %>% 
...
p4 <- sf_ra %>% 
  left_join(drg_hhi_rating_area ,"rating_area") %>% 
  filter(state %in% states_to_map) %>% 
  ggplot() + 
  geom_sf(aes(fill = hhi_comm_si)) +
  scale_fill_gradient2(low = scales::muted("blue"),mid = "white",high = scales::muted("red"),midpoint = 2500,limits = c(0,10000)) + 
