...
tn_czs <-
  county_to_cz %>% mutate(commuting_zone_id_2010 = paste0(commuting_zone_id_2010)) %>% filter(str_sub(fips_code,1,2) %in% c("47")) %>% pull(commuting_zone_id_2010)

cz_map <- sf::read_sf(here("output/tidy-mapping-files/commuting-zone/01_commuting-zone-shape-file.shp"))

p_cz <- cz_map %>% 
  filter(cz_id %in% tn_czs) %>% 
